"""
CSV/Excelのアウトライン表からPDFにアウトライン（しおり）を追加するスクリプト
...
使い方:
  python add_outline_from_table.py \
    ...
    --page_offset 0 \
    --sheet_name ""       # Excelの場合のみ（省略可）
    --page_col "page"     # 列名のカスタム（省略可）
    --class_col "class"   # 列名のカスタム（省略可）
    --title_col "title"   # 列名のカスタム（省略可）

メモ:
- CSV/Excel 側のページ番号が「本文の論理ページ（目次表記）」で、PDFの物理ページと
  ずれている場合は --page_offset で調整してください。
  例) 目次の p=1 が PDF では 22 ページ目(0始まり index=21) → --page_offset=+20 など
- class → レベル割り当てはデフォルト:
    "章"=1, "節"=2, "supplement"=3, "column"=3
  必要に応じて CLASS_TO_LEVEL を編集してください。
"""
